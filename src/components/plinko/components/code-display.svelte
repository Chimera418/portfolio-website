...
>
	{#if game.status !== "playing"}
		...
		<div class="flex flex-col items-center gap-4">
			...
			{#if game.status === "success"}
				<!-- Global modal takes over on success -->
			{:else}
				<div class="flex flex-col items-center gap-4">
					<span class="text-4xl md:text-5xl font-pixel tracking-widest text-red-400 drop-shadow-md uppercase text-center">Access denied</span>
					<span class="text-lg text-text-primary/80 font-mono text-center">
						You entered <span class="text-red-400 font-bold bg-red-400/20 px-2 py-1 rounded"> {game.entered.join("")} </span> — the code was <span class="text-primary font-bold bg-primary/20 px-2 py-1 rounded"> {game.target} </span>
					</span>
				</div>
	...
	{:else}
		...
		<div class="flex flex-col items-center gap-3">
			...
			<div class="flex flex-col items-center gap-2">
				<span class="text-xs font-pixel tracking-[0.2em] text-text-primary/40 uppercase">Enter your code</span>
				<div class="flex gap-2 text-2xl font-pixel tracking-[0.2em] text-text-primary/70">
